...
library LibIterableMapping {
    ...
    function getTransferDefinitionByName(IterableMapping storage self, string memory name) internal view returns (RegisteredTransfer memory) {
        require(!isEmptyString(name));
        require(nameExists(self, name));
        return self.transfers[name].transfer;
    }

    function getTransferDefinitionByIndex(IterableMapping storage self, uint256 index) internal view returns (RegisteredTransfer memory) {
