...
contract HashlockTransferAndWithdraw is TransferDefinition {
        ...
        returns (bool)
    {
        ...
        require(balance.amount[1] == 0, "HashlockTransferAndWithdraw: NONZERO_RECIPIENT_BALANCE");
        require(
            state.initiator != address(0) && state.responder != address(0),
            "HashlockTransferAndWithdraw: EMPTY_SIGNERS"
        );
        require(state.data != bytes32(0), "HashlockTransferAndWithdraw: EMPTY_DATA");
        require(state.nonce != uint256(0), "HashlockTransferAndWithdraw: EMPTY_NONCE");
        require(
            state.fee <= balance.amount[0],
            "HashlockTransferAndWithdraw: INSUFFICIENT_BALANCE"
        );
        require(
            ...
            "HashlockTransferAndWithdraw: INVALID_INITIATOR_SIG"
        );

        require(
            balance.amount[0] > 0,
            "HashlockTransferAndWithdraw: ZER0_SENDER_BALANCE"
        );

        require(
            balance.amount[1] == 0,
            "HashlockTransferAndWithdraw: NONZERO_RECIPIENT_BALANCE"
        );
        require(
            state.lockHash != bytes32(0),
            "HashlockTransferAndWithdraw: EMPTY_LOCKHASH"
        );
        require(
            state.expiry == 0 || state.expiry > block.timestamp,
            "HashlockTransferAndWithdraw: EXPIRED_TIMELOCK"
        );
