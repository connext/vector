...
contract CMCDeposit is CMCCore, AssetTransfer, ICMCDeposit {
  ...
  receive() external payable onlyOnProxy nonReentrant {}

  function getTotalDepositsAlice(address assetId) external override view onlyOnProxy nonReentrantView returns (uint256) {
  ...
  function getTotalDepositsBob(address assetId) external override view onlyOnProxy nonReentrantView returns (uint256) {
  ...
  function depositAlice(address assetId, uint256 amount) external override payable onlyOnProxy nonReentrant {
