...
contract Adjudicator is IAdjudicator {
  ...
  modifier onlyParticipant(CoreChannelState memory ccs) {
    require(
      msg.sender == ccs.participants[0] || msg.sender == ccs.participants[1],
      "ChannelFactory: msg.sender is not channel participant"
    );
    _;
  }

  function forceChannelConsensus(CoreChannelState memory ccs, bytes[2] memory signatures)
    public
    override
    onlyParticipant(ccs)
  {
    ...
    require(!inDefundPhase(dispute), "ChannelFactory forceChannelConsensus: Not allowed in defund phase");
    // TODO: check not defunded???
    require(dispute.nonce <= ccs.nonce, "ChannelFactory forceChannelConsensus: New nonce smaller than stored one");
  ...
  function defundChannel(CoreChannelState memory ccs) public override onlyParticipant(ccs) {
    // PSEUDOCODE: Please don't delete yet!
    ...
    address channelAddress = msg.sender; // getChannelAddress(ccs.participants[0], ccs.participants[1]);
    ChannelDispute storage dispute = channelDispute[channelAddress];
    require(inDefundPhase(dispute), "ChannelFactory defundChannel: Not in defund phase");
    require(!dispute.isDefunded, "ChannelFactory defundChannel: channel already defunded");
    ...
    /// NOTE: Temporarily commenting this code out -- it's fixed on another branch
  ...
  function forceTransferConsensus(CoreTransferState memory cts, bytes32[] memory merkleProofData)
    public
    override
  // TODO: Who should be able to call this?
  {
    ...
    require(inDefundPhase(dispute), "ChannelFactory forceTransferConsensus: Not in defund phase");
    ...
    // necessary?
    require(!transferDispute.isDefunded, "ChannelFactory forceTransferConsensus: transfer already defunded");
    // TODO: offchain-ensure that there can't be an overflow
  ...
  ) public override // TODO: Who should be able to call this?
  {
    // PSEUDOCODE: Please don't delete yet!
    // TransferDispute Memory transferDispute = transferDisputes(state.transferId)
    ...
    // TODO: check / simplify
    require(transferDispute.transferDisputeExpiry != 0, "ChannelFactory defundTransfer: transfer not yet disputed");
    require(!transferDispute.isDefunded, "ChannelFactory defundTransfer: transfer already defunded");
  ...
  function verifyMerkleProof(
    ...
    require(MerkleProof.verify(proof, root, leaf), "ChannelFactory: Merkle proof verification failed");
  }
