...
describe("CMCCore.sol", function () {
  ...
  describe("setup", async () => {
    ...
    it("should fail if it has already been setup", async () => {
      const setupTx = await channel.setup(alice.address, bob.address);
      await setupTx.wait();
      await expect(channel.setup(alice.address, bob.address)).revertedWith("CMCCore: ALREADY_SETUP");
    });

    it("should fail to setup if alice is not supplied", async () => {
      await expect(channel.setup(AddressZero, bob.address)).revertedWith("CMCCore: INVALID_PARTICIPANT");
    });
    ...
    it("should fail if alice == bob", async () => {
      await expect(channel.setup(alice.address, alice.address)).revertedWith("CMCCore: IDENTICAL_PARTICIPANTS");
    });
  });
