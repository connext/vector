...
const Auction: FC = () => {
  ...
  const runAuction = async (
    ...
    const res = await node.runAuction({
      amount,
      chainId,
      assetId,
  ...
  const setupChannel = async (supportedChains: number[], routerPublicIdentifier: string) => {
    let error: any | undefined;
    try {
      await node.channelSetup({ routerPublicIdentifier: routerPublicIdentifier });
    } catch (e) {
      console.error("Error initializing Browser Node:", jsonifyError(e));
      error = e;
    }
    ...
    if (error && shouldAttemptRestore) {
      ...
      for (const supportedChain of supportedChains) {
        ...
        if (!channelRes.getValue()) {
          const restoreChannelState = await node.restoreState({
    ...
    const channelsRes = await node.getStateChannels();
    ...
    const _channels = (
      await Promise.all(
        channelAddresses.map(async (c) => {
          const channelRes = await node.getStateChannel({ channelAddress: c });
    ...
    node.on(EngineEvents.DEPOSIT_RECONCILED, async (data) => {
      ...
      await updateChannel(node, data.channelAddress);
    });

    node.on(EngineEvents.CONDITIONAL_TRANSFER_CREATED, async (data) => {
          ...
          <>
            ...
            <Col span={18}>
              <Form
                ...
                labelCol={{ span: 6 }}
                onFinish={(vals) => {
                  runAuction(vals.amount, vals.chainId, vals.assetId, vals.recipientChainId, vals.recipientAssetId);
                }}
                initialValues={{
                  amount: "1",
                  chainId: 4,
              ...
              >
                ...
                <Form.Item name="chainId" label="sender chain id">
                  <Input />
                </Form.Item>

                <Form.Item name="assetId" label="sender asset id">
                  <Input />
                </Form.Item>

                <Form.Item name="recipientChainId" label="recipient asset id">
                  <Input />
                </Form.Item>

                <Form.Item name="recipientAssetId" label="recipient asset id">
                  <Input />
                </Form.Item>
