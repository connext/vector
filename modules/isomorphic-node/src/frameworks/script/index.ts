...
const makeDeposit = async (userId: ID, username: string, deposit: CreateChannelInput): Promise<void> => {
  const presenter: DepositPresenter = app.container.resolve<DepositPresenter>('depositPresenter');

  const response = await app.main.createChannel(deposit);
  const output = await presenter.present(response);
...
(async (): Promise<void> => {
  try {
    ...
    const megamanDeposit: CreateChannelInput = { userId: megamanId, value: 100 };

    await makeDeposit(megamanId, megaman.username, megamanDeposit);

    const zeroDeposit: CreateChannelInput = { userId: zeroId, value: 200 };
