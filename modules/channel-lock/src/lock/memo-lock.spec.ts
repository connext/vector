...
describe("MemoLock", () => {
  ...
  describe("with a common lock", () => {
    ...
    beforeEach(async () => {
      module = new MemoLock(redis, 5, TTL, 100);
  ...
  it("should expire locks in TTL order", async () => {
    const customModule = new MemoLock(redis, 5, 1000, 10000);
    ...
    setTimeout(() => customModule.pulse().catch((e) => console.error(`Caught error pulsing: ${e.stack}`)), 1200);
