...
if [[ "$isFresh" == "true" && -f "$backup_file" && "$PRODUCTION" == "true" ]]
then 
  ...
  log "Not restoring: Database exists ($isFresh) or no snapshots found ($backup_file) or not in prod-mode (prod=$PRODUCTION)"
fi
